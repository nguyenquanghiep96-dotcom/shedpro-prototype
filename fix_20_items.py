...
def format_name(filename):
    ...
    # Remove -e123456... suffix
    name = re.sub(r'-e\d+$', '', name)
    # Remove trailing numbers like -1, -2
    name = re.sub(r'-\d+$', '', name)
    name = name.replace('-', ' ')
    # Capitalize words properly, but keep original case mostly if they wanted something like "Barn with L-shape-Porch"
    # User example: Barn-with-L-shape-Porch-e1774955734611.png -> Barn with L-shape-Porch
    return name
...
for f in files:
    if f not in selected_images and len(selected_images) < 20:
...
for i, img in enumerate(selected_images):
    ...
    # Overrides for the top 4
    if i == 2:
        name = "Premium Shed"
    
    # path in HTML should be relative from public/v2/product/premium-shed/index.html
    # so ../../../v1/dxm5q5264srx2.cloudfront.net/...
    rel_path = img.replace("public/", "../../../")
    
    active_class = " active" if i == 2 else "" # Premium shed active by default
...
print("Added 20 items successfully.")
