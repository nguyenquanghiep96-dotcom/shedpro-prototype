...
# 1. Clean up old injected CSS and JS
content = re.sub(r'<style>\s*/\* Fix ic-close for popups \*/[\s\S]*?</style>', '', content)
content = re.sub(r'<style>\n/\* Fix ic-close \*/[\s\S]*?</style>', '', content)
...
content = re.sub(r'<script>\s*document\.addEventListener\("DOMContentLoaded", function\(\) \{\s*var selects = \[[\s\S]*?</script>', '', content)
content = re.sub(r'\n\s*\n\s*\n+', '\n\n', content)

# 2. Add demo options to HTML
...
"""
    # Find the end of ssb-configurator-size-area select
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    selects.forEach(function(cfg) {
        ...
        // Clean up any existing generated buttons
...
print("Added disabled options and extended script for height.")
